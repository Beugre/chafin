...
exports.simulateDailyCheck = exports.dailyPaymentReminders = exports.backfillDebtDocuments = exports.generateDebtAcknowledgment = exports.checkLoanStatus = exports.validateLoansIntegrity = exports.cleanupDuplicateSchedules = exports.testListLoans = exports.recalculateAllSchedules = exports.regenerateSchedules = exports.cleanupMalformedSchedules = void 0;
...
var cleanupSchedules_1 = require("./cleanupSchedules");
Object.defineProperty(exports, "cleanupMalformedSchedules", { enumerable: true, get: function () { return cleanupSchedules_1.cleanupMalformedSchedules; } });
Object.defineProperty(exports, "regenerateSchedules", { enumerable: true, get: function () { return cleanupSchedules_1.regenerateSchedules; } });
Object.defineProperty(exports, "recalculateAllSchedules", { enumerable: true, get: function () { return cleanupSchedules_1.recalculateAllSchedules; } });
var testLoans_1 = require("./testLoans");
Object.defineProperty(exports, "testListLoans", { enumerable: true, get: function () { return testLoans_1.testListLoans; } });
var cleanupDuplicates_1 = require("./cleanupDuplicates");
Object.defineProperty(exports, "cleanupDuplicateSchedules", { enumerable: true, get: function () { return cleanupDuplicates_1.cleanupDuplicateSchedules; } });
var validateLoansIntegrity_1 = require("./validateLoansIntegrity");
Object.defineProperty(exports, "validateLoansIntegrity", { enumerable: true, get: function () { return validateLoansIntegrity_1.validateLoansIntegrity; } });
var checkLoanStatus_1 = require("./checkLoanStatus");
Object.defineProperty(exports, "checkLoanStatus", { enumerable: true, get: function () { return checkLoanStatus_1.checkLoanStatus; } });
var debtAcknowledgment_2 = require("./debtAcknowledgment");
Object.defineProperty(exports, "generateDebtAcknowledgment", { enumerable: true, get: function () { return debtAcknowledgment_2.generateDebtAcknowledgment; } });
var backfillDebtDocs_1 = require("./backfillDebtDocs");
Object.defineProperty(exports, "backfillDebtDocuments", { enumerable: true, get: function () { return backfillDebtDocs_1.backfillDebtDocuments; } });
// ============================================================
// CONFIGURATION
// ============================================================
 ...
 */
exports.dailyPaymentReminders = (0, scheduler_1.onSchedule)({
    schedule: "0 9 * * *",
 ...
 */
exports.simulateDailyCheck = (0, https_1.onRequest)({
    cors: true,
}, async (req, res) => {
    try {
        const dateParam = req.query.date;
        const sendEmails = req.query.sendEmails === "true";
        const testEmail = req.query.email || "yoann.beugre1@gmail.com";
        if (!dateParam) {
...
async function runDailyChecks(now, isSimulation, testEmail) {
    const results = {
        loansProcessed: 0,
    ...
    const userMaxOverdueDays = new Map();
    ...
    for (const loanDoc of activeLoansSnapshot.docs) {
        ...
        const penaltiesAppliedThisLoan = [];
        for (const item of overdueSchedules) {
            ...
            const originalTotal = item.data.total;
        ...
        if (penaltiesAppliedThisLoan.length > 0) {
            ...
            if (!emailsDisabled) {
                const sent = await sendConsolidatedPenaltyEmail(loan, loanId, penaltiesAppliedThisLoan, debtUrl, isSimulation, testEmail);
                if (sent)
                    results.penaltyEmailsSent++;
            }
            else {
        ...
        if (overdueSchedules.length > 0) {
            ...
            if (shouldSend) {
                ...
                if (emailsDisabled) {
                    ...
                    const sent = await sendConsolidatedOverdueReminder(loan, loanId, overdueSchedules, loanMaxOverdueDays, now, isSimulation, testEmail);
                    if (sent)
            ...
            if (emailsDisabled) {
                ...
                const sent = await sendUpcomingReminder(loan, loanId, closest, now, isSimulation, testEmail);
                if (sent)
                    results.remindersSent++;
            }
        }
 ...
 */
async function sendConsolidatedOverdueReminder(loan, loanId, overdueSchedules, maxOverdueDays, now, isSimulation, testEmail) {
    try {
        ...
        const totalDue = overdueSchedules.reduce((sum, s) => sum + s.data.total, 0);
        const scheduleList = overdueSchedules
            .sort((a, b) => a.data.numero - b.data.numero)
            .map(s => `  • Échéance n°${s.data.numero} du ${formatDate(s.dueDate)} : ${s.data.total.toFixed(2)}€ (${s.days}j de retard)`)
            .join("\n");
        ...
        const emailSent = await sendEmailViaEmailJS({ email: userEmail, name: userName, subject, message, from_name: "Chafin Loans" }, isSimulation, testEmail);
 ...
 */
async function sendUpcomingReminder(loan, loanId, scheduleInfo, now, isSimulation, testEmail) {
    try {
        const todayKey = `${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, "0")}${String(now.getDate()).padStart(2, "0")}`;
        ...
        if (!userEmail)
            return false;
        const amount = scheduleInfo.data.total.toFixed(2);
 ...
 */
async function sendConsolidatedPenaltyEmail(loan, loanId, penalties, debtUrl, isSimulation, testEmail) {
    try {
        const now = new Date();
        const todayKey = `${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, "0")}${String(now.getDate()).padStart(2, "0")}`;
        ...
        const penaltyList = penalties
            ...
            .map(p => `  • Échéance n°${p.scheduleNumero} : ${p.originalTotal.toFixed(2)}€ → ${p.newTotal.toFixed(2)}€ (+${p.penaltyAmount.toFixed(2)}€ de pénalité, ${p.overdueDays}j de retard)`)
            .join("\n");
        const subject = isSimulation
 ...
 */
async function sendEmailViaEmailJS(emailData, isSimulation = false, testEmail) {
    ...
    try {
        const response = await axios_1.default.post(EMAILJS_CONFIG.apiUrl, {
 ...
 */
async function regenerateDebtAcknowledgment(loan, loanId) {
    try {
        const userDoc = await db.collection("users").doc(loan.userId).get();
        if (!userDoc.exists)
            return undefined;
        ...
        const pdfBuffer = await (0, debtAcknowledgment_1.buildDebtPDF)(loan, userData, schedules, loanId);
 ...
 */
async function checkAndDowngradeUserRisk(userId, maxOverdueDays) {
    try {
        const userDoc = await db.collection("users").doc(userId).get();
        if (!userDoc.exists)
            return false;
        const userData = userDoc.data();
        const currentLevel = userData.niveauConfiance;
        if (currentLevel !== null && currentLevel !== undefined && currentLevel <= 1.0) {
            return false;
        }
        const previousLevel = currentLevel !== null && currentLevel !== void 0 ? currentLevel : 3.0;
