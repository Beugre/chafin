...
exports.sendDebtAcknowledgmentEmail = exports.formatDateFR = exports.buildDebtPDF = exports.generateDebtAcknowledgment = void 0;
 ...
 */
exports.generateDebtAcknowledgment = (0, firestore_1.onDocumentUpdated)("loans/{loanId}", async (event) => {
    var _a, _b;
    ...
    if (before.statut === after.statut || after.statut !== "enCours")
        return;
    // Seulement depuis decaissementEffectue ou approuve
    if (before.statut !== "decaissementEffectue" && before.statut !== "approuve")
        return;
    ...
    try {
        ...
        // Envoyer par email
        if (userData.email) {
            await sendDebtAcknowledgmentEmail(userData.email, `${userData.prenom || ""} ${userData.nom || ""}`, after, loanId, downloadUrl);
        }
...
// GÉNÉRATION DU PDF
// ============================================================
function buildDebtPDF(loan, user, schedules, loanId) {
    return new Promise((resolve, reject) => {
        ...
        const montant = loan.montant || 0;
        const taux = loan.tauxAnnuel || 0;
        const duree = loan.dureeMois || 0;
        ...
        doc.moveDown(0.5);
        doc.fontSize(11).font("Helvetica");
        doc.text(`Je soussigné(e) ${user.prenom || ""} ${user.nom || ""}, ` +
        ...
        doc.moveDown(0.5);
        doc.fontSize(11).font("Helvetica");
        doc.text(`L'emprunteur s'engage à rembourser la somme totale de ${totalARembourser.toFixed(2)} € ` +
            `en ${duree} mensualités de ${mensualite.toFixed(2)} €, selon l'échéancier ci-dessous.`);
        doc.moveDown(0.5);
        doc.text("En cas de retard de paiement, une pénalité de 5% du montant de l'échéance " +
        ...
        if (schedules.length > 0) {
            ...
            for (const schedule of schedules) {
                ...
                doc.text(`${(schedule.principal || 0).toFixed(2)} €`, 180, y, { width: 80 });
                doc.text(`${(schedule.interet || 0).toFixed(2)} €`, 265, y, { width: 80 });
                doc.text(`${(schedule.total || 0).toFixed(2)} €`, 350, y, { width: 80 });
            ...
            doc.moveTo(50, doc.y).lineTo(515, doc.y).stroke();
            doc.moveDown(0.3);
            const totalPrincipal = schedules.reduce((sum, s) => sum + (s.principal || 0), 0);
            const totalInterets = schedules.reduce((sum, s) => sum + (s.interet || 0), 0);
            const totalGeneral = schedules.reduce((sum, s) => sum + (s.total || 0), 0);
            const yTotal = doc.y;
            doc.fontSize(9).font("Helvetica-Bold");
        ...
        doc.moveDown(4);
        doc.fontSize(8)
            .text("Ce document constitue une reconnaissance de dette au sens des articles 1326 et suivants du Code civil. " +
            "Généré automatiquement par la plateforme Chafin Loans.", 50, doc.y, { align: "center", width: 495 });
        doc.end();
    });
}
exports.buildDebtPDF = buildDebtPDF;
// ============================================================
// HELPERS
// ============================================================
...
async function sendDebtAcknowledgmentEmail(email, userName, loan, loanId, downloadUrl) {
    try {
        const montant = loan.montant || 0;
        const taux = loan.tauxAnnuel || 0;
        const duree = loan.dureeMois || 0;
        ...
        await axios_1.default.post(EMAILJS_CONFIG.apiUrl, {
...
exports.sendDebtAcknowledgmentEmail = sendDebtAcknowledgmentEmail;
//# sourceMappingURL=debtAcknowledgment.js.map